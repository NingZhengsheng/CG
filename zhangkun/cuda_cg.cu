...
__global__ void data(int *d_ptr, int *d_col, double *d_val, double *d_b,
                     double *d_x, double *d_r0, double *d_r1, double *d_p_sub,
...
void initData() {
  ...
  data<<<gridDim, blockDim>>>(d_ptr, d_col, d_val, d_b, d_x, d_r0, d_r1,
  ...
  cudaMemcpy(p_sub, d_p_sub, sizeof(double) * num_rows, cudaMemcpyDeviceToHost);
}
...
void initResidual() {
  ...
  int gridDim = (stride % blockDim) > 0 ? (stride / blockDim) + 1
                                         : (stride / blockDim);
  residualD<<<gridDim, blockDim>>>(num_rows, d_r1, d_s, offset, stride);
