...
int main(int argc, char **argv) {
  ...
  __host__ void calBeta(cublasHandle_t handle2, double *d_r1,double *d_r0);
  // time
  ...
  while (iteration < MAX_ITERATION) {
    ...
    calAlpha(handle2, d_r0, d_r0, d_Ap, d_p);
    calXAndR(handle2, d_p, d_Ap, d_r0, d_r1, d_x);
    calBeta(handle2, d_r1, d_r0);
    cublasDaxpy(handle2, num_rows, &beta, d_p, 1, d_r1, 1);
...
__host__ void initData(cusparseHandle_t handle, cublasHandle_t handle2,
  ...
  cublasDcopy(handle2, num_rows, d_b, 1, d_p, 1);
}
...
__host__ void calAlpha(cublasHandle_t handle2, double *d_r0, double *d_z0,
                       double *d_Ap, double *d_p) {
  double s, m;
  cublasDdot(handle2, num_rows, d_r0, 1, d_r0, 1, &s);
...
__host__ void calBeta(cublasHandle_t handle2, double *d_r1,
                      double *d_r0) {
  double s, m;
  cublasDdot(handle2, num_rows, d_r1, 1, d_r1, 1, &s);
  cublasDdot(handle2, num_rows, d_r0, 1, d_r0, 1, &m);
  beta = s / m;
