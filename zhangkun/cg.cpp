...
int main(int argc, char **argv)
{
	...
	while (iteration < MAX_ITERATION ) //02
	{
		...
		spmvtime += (double)(spmvte - spmvts) / CLOCKS_PER_SEC;
		dotts = clock();
		alpha = multiVector(r0, r0) / multiVector(Ap, p);
		dotte = clock();
		dottime += (double)(dotte - dotts) / CLOCKS_PER_SEC;
		...
		dotts = clock();
		beta = multiVector(r1, r1) / multiVector(r0, r0);
		dotte = clock();
		dottime += (double)(dotte - dotts) / CLOCKS_PER_SEC;
...
{
	ofstream fout;
	string base = "C:/Users/Nathan/Desktop/" + filename + "_residual0.xls";
	fout.open(base.c_str(), ios::app);
	if (a == 1)
		fout << "iteration\tresidual0" << endl;
